...
target_mods = find_lora_targets(controlnet)
print("LoRA 대상(샘플):", target_mods[:20], "...")

lora_cfg = LoraConfig(r=4, lora_alpha=16, bias="none", target_modules=target_mods)
controlnet = get_peft_model(controlnet, lora_cfg).to(device)
controlnet.print_trainable_parameters()
controlnet.train()  # ← 학습 모드
...
# 주의: attention_slicing은 메모리 절약 대신 속도 하락 가능 → 메모리 여유 없을 때만 유지
# pipe.enable_attention_slicing()
pipe.unet.to(memory_format=torch.channels_last)

pipe.controlnet.enable_gradient_checkpointing()  # 메모리 절약(속도 약간↓)
...
def get_text_embeds(class_ids, defect_ids):
    # class_ids/defect_ids: CPU LongTensor (B,)
...
def prep_mask(mask_bf16):
    # mask_bf16: (B,1,H,W) or (B,C,H,W) in [-1,1] or [0,1] → 표준화
    if mask_bf16.dim() == 3:
    ...
    # 만약 원래 [0,1]이면 아래 줄로 통일 (필요시 주석 해제)
    # mask_bf16 = mask_bf16.clamp(0,1) * 2 - 1
    return mask_bf16

# ───────────── OPTIMIZER ─────────────
optimizer = AdamW8bit(filter(lambda p: p.requires_grad, pipe.controlnet.parameters()), lr=LR)
...
for epoch in range(1, EPOCHS + 1):
    ...
    for batch in tqdm(loader, desc=f"Epoch {epoch}/{EPOCHS}"):
        ...
        ctrl_out = pipe.controlnet(
            ...
            controlnet_cond=cond,           # ← prep_mask 필요 없음
            return_dict=True,
        )
        lat_pred = pipe.unet(
            sample=noisy_lat,
        ...
        if global_step % SAVE_STEPS == 0:
            controlnet.save_pretrained(f"{OUTPUT_DIR}/lora_step{global_step}")
